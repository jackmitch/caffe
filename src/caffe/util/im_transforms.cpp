...
void UpdateBBoxByResizePolicy(const ResizeParameter& param,
                              const int old_width, const int old_height,
                              NormalizedBBox* bbox, int resize_mode_used) {
  float new_height = param.height();
  float new_width = param.width();
  float orig_aspect = static_cast<float>(old_width) / old_height;
  float new_aspect = new_width / new_height;
  ...
  float y_max = bbox->ymax() * old_height;
  float padding;
  caffe::ResizeParameter::Resize_mode resize_mode = param.resize_mode(resize_mode_used);
  switch (resize_mode) {
    case ResizeParameter_Resize_mode_WARP:
      x_min = std::max(0.f, x_min * new_width / old_width);
      x_max = std::min(new_width, x_max * new_width / old_width);
...
cv::Mat ApplyResize(const cv::Mat& in_img, const ResizeParameter& param, int *resize_mode_used) {
  cv::Mat out_img;
  ...
  int num_resize_modes = param.resize_mode_size();
  caffe::ResizeParameter::Resize_mode resize_mode = ResizeParameter_Resize_mode_WARP;
  if (num_resize_modes > 0) {
    vector<float> probs(num_resize_modes, 1.f / num_resize_modes);
    int prob_num = roll_weighted_die(probs);
    resize_mode = param.resize_mode(prob_num);
    if (resize_mode_used) {
      *resize_mode_used = prob_num;
    }
  }
