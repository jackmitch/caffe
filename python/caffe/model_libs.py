...
def VGG_M_NetBody(net, from_layer, need_fc=True, fully_conv=False, reduced=False,
	...
	net.conv1 = L.Convolution(net[from_layer], num_output=96, pad=1, kernel_size=7, stride=2, **kwargs)
	net.relu1 = L.ReLU(net.conv1, in_place=True)
	net.norm1 = L.LRU(net.relu1, local_size=5, alpha=0.0005, beta=0.75, k=2)
	net.pool1 = L.Pooling(net.norm1, pool=P.Pooling.MAX, kernel_size=3, stride=2)
	
	net.conv2 = L.Convolution(net.pool1, num_output=256, pad=1, kernel_size=5, stride=2)
	net.relu2 = L.ReLU(net.conv2, in_place=True)
	net.norm2 = L.LRU(net.relu2, local_size=5, alpha=0.0005, beta=0.75, k=2)
	net.pool1 = L.Pooling(net.norm2, pool=P.Pooling.MAX, kernel_size=3, stride=2)
	
	net.conv3 = L.Convolution(net.pool2, num_output=512, pad=1, kernel_size=3, stride=1)
	net.relu3 = L.ReLU(net.conv3, in_place=True)

	net.conv4 = L.Convolution(net.relu3, num_output=512, pad=1, kernel_size=3, stride=1)
	net.relu4 = L.ReLU(net.conv4, in_place=True)
	
	net.conv5 = L.Convolution(net.relu4, num_output=512, pad=1, kernel_size=3, stride=1)
	net.relu5 = L.ReLU(net.conv5, in_place=True)
	
    if need_fc:
        if dilated:
			net.pool5 = L.Pooling(net.relu5, pool=P.Pooling.MAX, pad=1, kernel_size=3, stride=1)
        else:
			net.pool5 = L.Pooling(net.relu5, pool=P.Pooling.MAX, kernel_size=3, stride=2)

        if fully_conv:
            if dilated:
                if reduced:
                    net['fc6-conv'] = L.Convolution(net[name], num_output=1024, pad=4, kernel_size=3, dilation=4, **kwargs)
                else:
                    net['fc6-conv'] = L.Convolution(net[name], num_output=4096, pad=4, kernel_size=6, dilation=2, **kwargs)
            else:
                if reduced:
                    net['fc6-conv'] = L.Convolution(net[name], num_output=1024, pad=2, kernel_size=3, dilation=2, **kwargs)
                else:
                    net['fc6-conv'] = L.Convolution(net[name], num_output=4096, pad=2, kernel_size=6, **kwargs)
            ...
            if reduced:
                net['fc7-conv'] = L.Convolution(net.relu6, num_output=1024, kernel_size=1, **kwargs)
            else:
                net['fc7-conv'] = L.Convolution(net.relu6, num_output=4096, kernel_size=1, **kwargs)
            net.relu7 = L.ReLU(net['fc7-conv'], in_place=True)
            if dropout:
...
def VGGNetBody(net, from_layer, need_fc=True, fully_conv=False, reduced=False,
    ...
    if need_fc:
        ...
        if fully_conv:
            if dilated:
                if reduced:
                    net['fc6-conv'] = L.Convolution(net[name], num_output=1024, pad=6, kernel_size=3, dilation=6, **kwargs)
                else:
                    net['fc6-conv'] = L.Convolution(net[name], num_output=4096, pad=6, kernel_size=7, dilation=2, **kwargs)
            else:
                if reduced:
                    net['fc6-conv'] = L.Convolution(net[name], num_output=1024, pad=3, kernel_size=3, dilation=3, **kwargs)
                else:
                    net['fc6-conv'] = L.Convolution(net[name], num_output=4096, pad=3, kernel_size=7, **kwargs)
