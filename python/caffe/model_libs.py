...
def VGG_M_NetBody(net, from_layer, need_fc=True, fully_conv=False, reduced=False,
    ...
    net.norm1 = L.LRN(net.relu1, local_size=5, alpha=0.0005, beta=0.75, k=2)
    ...
    net.norm2 = L.LRN(net.relu2, local_size=5, alpha=0.0005, beta=0.75, k=2)
    ...
    if need_fc:
        ...
        if fully_conv:
            if dilated:
                if reduced:
                    net['fc6-conv'] = L.Convolution(net.pool5, num_output=1024, pad=4, kernel_size=3, dilation=4, **kwargs)
                else:
                    net['fc6-conv'] = L.Convolution(net.pool5, num_output=4096, pad=4, kernel_size=6, dilation=2, **kwargs)
            else:
                if reduced:
                    net['fc6-conv'] = L.Convolution(net.pool5, num_output=1024, pad=2, kernel_size=3, dilation=2, **kwargs)
                else:
                    net['fc6-conv'] = L.Convolution(net.pool5, num_output=4096, pad=2, kernel_size=6, **kwargs)
